...
export function apply(ctx) {
  ...
  // Session id resolution, tolerant across runtimes: older agents expose
  // `sessionId` directly, current ones (>= 0.1.1-rc) expose `session.id`;
  // the agent id is the last-resort key so per-agent edge tracking still works.
  const sessionIdOf = (agent) => {
    if (!agent) return ''
    const id = agent.sessionId !== undefined && agent.sessionId !== null
      ? agent.sessionId
      : agent.session !== undefined && agent.session !== null && agent.session.id !== undefined
        ? agent.session.id
        : agent.id
  ...
  ctx.on('approval/request', (req, next) => {
    try {
      const sessionId = sessionIdOf(req && req.agent)
      push({
        kind: 'approval',
        sessionId,
